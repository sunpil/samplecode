    ...
    render() {
        ...
        return (
            <div className="smart-form">
                <section className="col col-5 form-inline" style={{paddingLeft:0}}>
                    <label className="label">전체</label>
                    <label className="select select-multiple" >
                        <select id={this.selectListId} multiple className="custom-scroll" style={{height:110}}>
                            {this.state.selectList && this.state.selectList.length > 0 && this.state.selectList.map((item) => {
                                if(!selectedMap.has(item[key_name])){
                ...
                <section className="col col-2 dual-list-btn-group">
                    <button type="button" className="btn move btn-default daul-list-btn" onClick={() => { this._onSelect() }}>
                        <i className="glyphicon glyphicon-arrow-right"></i>
                    </button><br />
                    <button type="button" className="btn move btn-default daul-list-btn" onClick={() => { this._deSelect() }}>
                        <i className="glyphicon glyphicon-arrow-left"></i>
                    </button>
                </section>
                <section className="col col-5 form-inline" style={{paddingRight:0}}>
                    <label className="label">선택</label>
                    <label className="select select-multiple" >
                        <select id={this.selectedListId} multiple className="custom-scroll" style={{height:110}}>
